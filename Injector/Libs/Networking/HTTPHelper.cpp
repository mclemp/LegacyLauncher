...
HttpResponse HTTPHelper::SendBasicRequest(const std::wstring& address, const std::wstring& path, int port, const wchar_t* method, const std::string& requestData, const std::wstring contentType) {
	std::vector<std::wstring> headers;
	headers.push_back(L"Content-Type: " + contentType);

	return HTTPHelper::SendAdvancedRequest(address, path, port, method, requestData, headers);
}

HttpResponse HTTPHelper::SendAdvancedRequest(const std::wstring& address, const std::wstring& path, int port, const wchar_t* method, const std::string& requestData, const std::vector<std::wstring>& headers) {
